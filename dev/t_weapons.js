...
(async()=>{
  ...
  let s=await curtain(b); const X=s.cur, Y=s.cur==='host'?'guest':'host'; console.log('first:', X, 'weapons shown:', await b.eval(`document.querySelectorAll('.weapons button.wpn').length`), await b.eval(`[...document.querySelectorAll('.weapons button.wpn')].map(x=>x.textContent).join(' | ')`));
  // --- hover preview ---
  ...
  console.log('mine on empty water stays dormant:', await b.eval(`Underway.UI.S.game.players['${Y}'].shots.filter(x=>x.weapon==='mine').length===0`));
  ...
  s=await curtain(b); const ycar=await cells(b,Y,0); const row=ycar[0].y; await pickWeapon(b,'torpedo'); console.log('torpedo btn:', await b.eval(`document.querySelector('.weapons .wgroup:nth-child(3) button.wpn').textContent`));
  ...
  // --- Y: second mine laid right on X's patrol boat: detonates on contact ---
  s=await curtain(b); if((await st(b)).cur!==Y){ await pickWeapon(b,'shell'); await tapEnemy(b,P(9,1)); await b.click('#btn-fire'); s=await settle(b); s=await curtain(b); }
  const xp=await cells(b,X,4); const minesBefore=await b.eval(`Underway.UI.S.game.players['${Y}'].shots.filter(x=>x.weapon==='mine').length`);
  await pickWeapon(b,'mine'); console.log('mine btn:', await b.eval(`document.querySelector('.weapons .wgroup:nth-child(4) button.wpn').textContent`)); await tapEnemy(b,xp[0]); await b.click('#btn-fire'); await b.wait(600); await b.shot(__dirname+'/w_mine_contact.png'); s=await settle(b);
  console.log('contact mine: new mine shots', (await b.eval(`Underway.UI.S.game.players['${Y}'].shots.filter(x=>x.weapon==='mine').length`))-minesBefore, '| result:', await b.eval(`JSON.stringify(Underway.UI.S.game.players['${Y}'].shots.filter(x=>x.weapon==='mine').slice(-1)[0])`), '| X patrol hits:', await b.eval(`JSON.stringify(Underway.UI.S.game.players['${X}'].fleet[4].hits)`), '| turn passed:', s.cur===X);
  s=await curtain(b);
