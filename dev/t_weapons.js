...
async function pickWeapon(b,id){ const i=['shell','mg','mine','radar','airstrike'].indexOf(id)+1; await b.click(`.weapons button:nth-child(${i})`); await b.wait(80); return b.eval(`Underway.UI.S.weapon`); }
...
(async()=>{
  ...
  s=await curtain(b); const xbb=await cells(b,X,1); const xbShip=await b.eval(`Underway.UI.S.game.players['${X}'].fleet[1]`); const horiz=(xbShip.heading==='E'||xbShip.heading==='W');
