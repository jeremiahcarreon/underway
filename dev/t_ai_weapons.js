...
async function run(mobile){
  ...
  for(let t=0;t<30;t++){ if(s.winner) break; const cell=await b.eval(`(()=>{const S=Underway.UI.S; return Underway.AI.admiralShot(S.game.players.host.shots, Underway.AI.initMemory('admiral'), Underway.RNG(${t+5}));})()`); await pickWeapon(b,'shell'); await tapEnemy(b,cell); if(await b.eval(`document.getElementById('btn-fire').disabled`)){ console.log('fire disabled at', JSON.stringify(cell)); break; } await b.click('#tray-fire'); s=await settle(b); }
