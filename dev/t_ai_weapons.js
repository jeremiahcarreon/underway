const {Browser,cellXY,waitFor,URL,tapEnemy,showTab,pickWeapon,setLine}=require('./cdp.js'); const P=(x,y)=>({x,y});
...
async function run(mobile){
  ...
  if(mobile){ await b.click('#tab-enemy'); await b.wait(200); await b.shot(__dirname+'/w_actions_mobile.png'); console.log('mobile hscroll ok:', await b.eval(`document.documentElement.scrollWidth<=window.innerWidth`)); }
  else {  await b.shot(__dirname+'/w_actions_desktop.png'); }
  // MG on the AI carrier
  const c=await aiCells(b,0); await pickWeapon(b,'mg',1); for(const cell of [c[0],c[1],P(9,9),P(0,9)]) await tapEnemy(b,cell); await b.click('#tray-fire'); s=await settle(b);
  ...
  const dcells=await aiCells(b,2); await pickWeapon(b,'shell'); await tapEnemy(b,dcells[1]); await b.click('#tray-fire'); s=await settle(b);
  ...
  await pickWeapon(b,'mine'); await tapEnemy(b,ahead); await b.click('#tray-fire'); s=await settle(b);
  await pickWeapon(b,'mine'); await tapEnemy(b,behind); await b.click('#tray-fire'); s=await settle(b);
  console.log('mines laid:', await b.eval(`Underway.UI.S.game.players.host.mines.length`), 'mines left:', await b.eval(`Underway.Rules.weapon(Underway.UI.S.game.players.host,'mine').usesRemaining`));
  // radar with an undamaged ship
  const und=await b.eval(`Underway.UI.S.game.players.host.fleet.findIndex(s=>!s.sunk&&Underway.Rules.hitCount(s)===0)`); if(und>=0){ await pickWeapon(b,'radar',und); await tapEnemy(b,P(4,4)); await b.click('#tray-fire'); s=await settle(b); console.log('radar done, contacts:', await b.eval(`Underway.UI.S.game.log.filter(e=>e.kind==='radar').map(e=>e.count).join(',')`)); }
  // air strike if carrier undamaged
  const carOk=await b.eval(`(()=>{const c=Underway.UI.S.game.players.host.fleet[0]; return !c.sunk&&Underway.Rules.hitCount(c)===0;})()`); if(carOk){ const bb=await aiCells(b,1); await pickWeapon(b,'airstrike'); const bs=await b.eval(`Underway.UI.S.game.players.guest.fleet[1]`); const horiz=bs.heading==='E'||bs.heading==='W'; const anchor=bb.slice().sort((p,q)=>p.x-q.x||p.y-q.y)[0]; await tapEnemy(b,anchor); await setLine(b,horiz); await b.click('#tray-fire'); s=await settle(b); console.log('airstrike:', await b.eval(`JSON.stringify(Underway.UI.S.game.players.host.shots.filter(x=>x.weapon==='airstrike').map(x=>x.result))`)); } else console.log('carrier damaged, strike skipped');
  // 30 shell turns with the admiral helper; watch for mine hits by the AI
  for(let t=0;t<30;t++){ if(s.winner) break; const cell=await b.eval(`(()=>{const S=Underway.UI.S; return Underway.AI.admiralShot(S.game.players.host.shots, Underway.AI.initMemory('admiral'), Underway.RNG(${t+5}));})()`); await pickWeapon(b,'shell'); await tapEnemy(b,cell); if(await b.eval(`(!document.getElementById('tray-fire')||document.getElementById('tray-fire').disabled)`)){ console.log('fire disabled at', JSON.stringify(cell)); break; } await b.click('#tray-fire'); s=await settle(b); }
