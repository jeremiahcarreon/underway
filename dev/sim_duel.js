...
function human(g,rng,style,mem){
  ...
  if(style.punish){ const sg=(g.players.host.sightings||[]).filter(x=>x.cells&&g.turn-x.turn<=8).slice(-1)[0]; if(sg){ const tk=new Map(); g.players.host.shots.forEach(x=>tk.set(x.cell.x+','+x.cell.y,x)); const c=sg.cells.find(c=>{ const t=tk.get(c.x+','+c.y); return !t||t.turn<sg.turn; }); if(c){ const r=Rules.fire(g,'host',c); if(r.ok) return r.game; } } }
...
for(const level of ['hunter','admiral','fleet']) for(const [name,style] of [['static hunter-human',{}],['evasive human',{evade:true}],['evasive + weapons',{evade:true,weapons:true}],['evasive, punishes reveals',{evade:true,weapons:true,punish:true}]]){ const rs=[]; for(let s=1;s<=N;s++) rs.push(duel(s,level,style)); const wins=rs.filter(r=>r.w==='guest').length; const done=rs.filter(r=>r.w); console.log(level.padEnd(8), name.padEnd(22), 'AI wins', (wins+'/'+N).padEnd(6), 'avg turns', Math.round(done.reduce((a,r)=>a+r.turns,0)/Math.max(1,done.length)), 'unfinished', N-done.length); }
