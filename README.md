...
- **② Attack.** Tap a square, then tap the same square again (or FIRE). The weapon button opens the weapon list with status and a one-line description; the machine gun and radar ask which ship inside that list. Torpedo and air strike put their direction choice on the map itself: a torpedo shows an arrow at each end of the row and the column, an air strike shows a TURN handle where the line would swing to. Tap one to switch; the tray never grows over the map. The machine gun numbers its squares and fires from the tray.
